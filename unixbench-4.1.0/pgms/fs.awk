...
/write/ { if (!fail) {
	       ...
	       w2+=$1*$1;
	       w_product += log($1); 
		printf("File Write %d bufsize %d maxblocks,%d,%.1f,KBps\n", bufsize, maxblocks, iter, $1)
	       }
        }
/read/  { if (!fail) {
	       r+=$1;
	       r2+=$1*$1;
	       r_product += log($1); 
		printf("File Read %d bufsize %d maxblocks,%d,%.1f,KBps\n", bufsize, maxblocks, iter, $1)
	       }
        }
/copy/  { if (!fail) {
	       c+=$1;
	       c2+=$1*$1;
	       c_product += log($1);
		printf("File Copy %d bufsize %d maxblocks,%d,%.1f,KBps\n", bufsize, maxblocks, iter, $1)
	       }
	}
END {
	if (ok > 0) {
